...
from t5_utils import initialize_model, initialize_optimizer_and_scheduler, save_model, load_model_from_checkpoint, setup_wandb, save_training_state, load_training_state
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    ...
    if args.resume:
        state = load_training_state(checkpoint_dir, model, optimizer, scheduler)
    ...
    for epoch in range(start_epoch, args.max_n_epochs):
        ...
        save_training_state(checkpoint_dir, model, optimizer, scheduler, epoch, best_f1, epochs_since_improvement)
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    '''
    Evaluation loop: computes loss and generates SQL queries, then computes metrics.
    '''
    model.eval()
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    '''
    Inference on the test set: generate SQL queries and save them with associated records.
    '''
    model.eval()
    tokenizer = T5TokenizerFast.from_pretrained('google-t5/t5-small')
    all_generated_queries = []
    ...
    save_queries_and_records(all_generated_queries, model_sql_path, model_record_path)
    print(f"Saved {len(all_generated_queries)} test predictions to {model_sql_path} and {model_record_path}")
